...
public class Parser
{
    ...
    static void Get ()
    {
        for (;;)
        {
            ...
            if (la.kind <= maxT)
            {
                ++errDist;    /* ML return changed to break */
                break;
            }
            if (la.kind == 43)
            {
    ...
    static void SimSet(out BitArray s)
    {
        ...
        if (la.kind == 1)
        {
            ...
            if (c == null) SemErr("undefined name");
            else s.Or(c.set);
