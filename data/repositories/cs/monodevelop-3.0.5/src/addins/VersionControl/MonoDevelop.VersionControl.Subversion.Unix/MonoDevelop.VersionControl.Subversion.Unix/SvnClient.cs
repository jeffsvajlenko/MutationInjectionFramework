...
class SvnClient : SubversionVersionControl
{
    ...
    private VersionStatus ConvertStatus (LibSvnClient.NodeSchedule schedule, LibSvnClient.VersionStatus status)
    {
        switch (schedule)
        {
        case LibSvnClient.NodeSchedule.Add:
            return VersionStatus.Versioned | VersionStatus.ScheduledAdd;
        case LibSvnClient.NodeSchedule.Delete:
            return VersionStatus.Versioned | VersionStatus.ScheduledDelete;
        case LibSvnClient.NodeSchedule.Replace:
            return VersionStatus.Versioned | VersionStatus.ScheduledReplace;
        }

        switch (status)
        {
        case LibSvnClient.VersionStatus.None:
            return VersionStatus.Versioned;
        case LibSvnClient.VersionStatus.Normal:
            return VersionStatus.Versioned;
        case LibSvnClient.VersionStatus.Unversioned:
            return VersionStatus.Unversioned;
        case LibSvnClient.VersionStatus.Modified:
            return VersionStatus.Versioned | VersionStatus.Modified;
        case LibSvnClient.VersionStatus.Merged:
            return VersionStatus.Versioned | VersionStatus.Modified;
        case LibSvnClient.VersionStatus.Conflicted:
            return VersionStatus.Versioned | VersionStatus.Conflicted;
        case LibSvnClient.VersionStatus.Ignored:
            return VersionStatus.Unversioned | VersionStatus.Ignored;
        case LibSvnClient.VersionStatus.Obstructed:
            return VersionStatus.Versioned;
        case LibSvnClient.VersionStatus.Added:
            return VersionStatus.Versioned | VersionStatus.ScheduledAdd;
        case LibSvnClient.VersionStatus.Deleted:
            return VersionStatus.Versioned | VersionStatus.ScheduledDelete;
        case LibSvnClient.VersionStatus.Replaced:
            return VersionStatus.Versioned | VersionStatus.ScheduledReplace;
        }
    ...
    private class LogCollector
    {
        ...
        public IntPtr Func (IntPtr baton, IntPtr apr_hash_changed_paths, int revision, IntPtr author, IntPtr date, IntPtr message, IntPtr pool)
        {
            ...
            while (item != IntPtr.Zero)
            {
                ...
                {
                case 'A':
                    ac = RevisionAction.Add;
                    break;
                case 'D':
                    ac = RevisionAction.Delete;
                    break;
                case 'R':
                    ac = RevisionAction.Replace;
                    break;
                default:
                    ac = RevisionAction.Modify;
                    break; // should be an 'M'
                }
