...
public class NiCad {
	...
	public List<Clone> runTool(String systemDirectory, String toolDirectory, String language, String granularity, String configFile) {
		...
		try {
			...
			//e.printStackTrace();
			//System.out.println("NiCad: Error, process interrupted. Probable Cause: NiCad not setup correctly, or tool incorrectly specified in database.");
			return null;
		} catch (InterruptedException e) {
			//e.printStackTrace();
			//System.out.println("NiCad: Error, process interrupted. Probable Cause: Unknown.");
			return null;
		}
		if(pretval != 0) {
			//System.out.println("NiCad: Error, NiCad ended with error. Probable Cause: Unknown, check log in " + systemDirectory + ".");
			return null;
		}
		...
		if(!results.exists()) {
			//System.out.println("NiCad: Could not find results file.  If changed config, need to alter NiCad tool runner!");
		}
		Scanner s;
		try {
			...
			//e.printStackTrace();
			//System.out.println("NiCad: Could not read results file.  permissions error?");
			return null;
		}
